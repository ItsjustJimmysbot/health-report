...
def setup_gmail_oauth2():
    ...
    # 6. 保存配置（保留用户原有 provider 优先级，不强制覆盖）
    oauth2_update = {
        "enabled": True,
        "provider": "gmail",
        "client_id": client_id,
    ...
    # 合并配置
    email_config = existing_config.get('email_config')
    if not isinstance(email_config, dict):
        email_config = {}
        existing_config['email_config'] = email_config

    default_priority = ["oauth2", "smtp", "mail_app", "local"]
    current_priority = email_config.get('provider_priority', [])
    if not isinstance(current_priority, list) or not current_priority:
        current_priority = default_priority.copy()
    else:
        # 保持原顺序，仅补齐缺失项
        for p in default_priority:
            if p not in current_priority:
                current_priority.append(p)
    email_config['provider_priority'] = current_priority

    oauth2_config = email_config.get('oauth2')
    if not isinstance(oauth2_config, dict):
        oauth2_config = {}
    oauth2_config.update(oauth2_update)
    email_config['oauth2'] = oauth2_config
