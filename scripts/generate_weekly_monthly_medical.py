#!/usr/bin/env python3
"""
周报和月报生成器 - V5.0 Medical Dashboard版
...
def generate_trend_chart(dates, hrv_values, steps_values, sleep_values, chart_type='weekly'):
    ...
    if not dates or not hrv_values:
        return '<div style="text-align:center;color:#999;padding:40px;">暂无趋势数据</div>'
...
def generate_weekly_report(start_date, end_date, ai_analysis, template):
    ...
    for date in week_dates:
        ...
        if data:
            row = f"""<tr>
                ...
                <td><span class="rating rating-good">良好</span></td>
            </tr>"""
    ...
    html = html.replace('{{WEEK_RANGE}}', f"第{start.isocalendar()[1]}周")
    ...
    html = html.replace('{{WORKOUT_RATIO}}', f"{workout_days}/{len(weekly_data)}天")
...
def generate_monthly_report(year, month, ai_analysis, template):
    ...
    html = html.replace('{{AVG_SLEEP}}', f"{avg_sleep:.1f}")
    html = html.replace('{{WORKOUT_DAYS}}', str(workout_days))
    html = html.replace('{{WORKOUT_RATIO}}', f"{workout_days}/{len(monthly_data)}天")
...
def generate_recommendations_html(recommendations):
    ...
    for rec in recommendations:
        ...
        p_label = priority_labels.get(rec.get('priority', 'medium'), '建议')
        # 将\n替换为<br>以正确显示换行
...
def main():
    ...
    if report_type == 'weekly':
        ...
        # 加载模板
        template_path = TEMPLATE_DIR / 'WEEKLY_TEMPLATE_MEDICAL.html'
        ...
        # 加载模板
        template_path = TEMPLATE_DIR / 'MONTHLY_TEMPLATE_MEDICAL.html'
        with open(template_path, 'r', encoding='utf-8') as f:
            template = f.read()
