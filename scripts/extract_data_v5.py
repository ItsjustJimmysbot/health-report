...
# V5.5.0: 从 config.json 读取路径配置，支持多语言
def load_config():
    """加载配置文件"""
...
def parse_sleep_data_v5(date_str, health_dir=None):
    """V5.0: 使用sleepStart字段，严格时间窗口筛选 - V5.5.0-fix: 支持路径传入"""
    date = datetime.strptime(date_str, '%Y-%m-%d')
    next_date = (date + timedelta(days=1)).strftime('%Y-%m-%d')
    
    # V5.5.0-fix: 使用传入的路径或全局默认路径
...
def extract_daily_data(date_str, health_dir=None, workout_dir=None):
    """提取完整的一天数据 - V5.5.0-fix: 支持多成员路径传入"""
    date = datetime.strptime(date_str, '%Y-%m-%d')
    next_date = (date + timedelta(days=1)).strftime('%Y-%m-%d')
