...
def extract_daily_data(date_str, health_dir=None, workout_dir=None, user_profile=None, sleep_config=None):
    ...
    result = {
        ...
        'hrv': {
            'value': round(hrv_raw, 1) if hrv_raw is not None else None,
            'measurement_count': hrv_measurement_count,  # 测量次数，不是评分
            'unit': 'ms'
        },
        'resting_hr': {
            'value': round(resting_hr_raw) if resting_hr_raw is not None else None
        },
        'steps': int(steps),
        ...
        'physical_effort': round(physical_effort, 2) if physical_effort is not None else None,
        'breathing_disturbances': round(breathing_disturbances, 1) if breathing_disturbances is not None else None,
        ...
        'stair_speed_up': round(stair_speed_up, 2) if stair_speed_up is not None else None,
        'running_speed': round(running_speed, 2) if running_speed is not None else None,
        'running_power': round(running_power, 1) if running_power is not None else None,
        'running_stride_length': round(running_stride_length, 1) if running_stride_length is not None else None,
        'running_ground_contact_time': round(running_ground_contact_time, 1) if running_ground_contact_time is not None else None,
        'running_vertical_oscillation': round(running_vertical_oscillation, 1) if running_vertical_oscillation is not None else None,
        'walking_speed': round(walking_speed, 2) if walking_speed is not None else None,
        'walking_step_length': round(walking_step_length, 1) if walking_step_length is not None else None,
        'walking_heart_rate_average': round(walking_heart_rate_average, 1) if walking_heart_rate_average is not None else None,
        'walking_asymmetry_percentage': round(walking_asymmetry_percentage, 2) if walking_asymmetry_percentage is not None else None,
        'walking_double_support_percentage': round(walking_double_support_percentage, 2) if walking_double_support_percentage is not None else None,
        'headphone_audio_exposure': round(headphone_audio_exposure, 1) if headphone_audio_exposure is not None else None,
        'environmental_audio_exposure': round(environmental_audio_exposure, 1) if environmental_audio_exposure is not None else None,
        'sleep': {
