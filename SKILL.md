...
# Health Report V5.7.1

此 Skill 能够将 Apple Health 的原始导出数据转化为深度、个性化的健康报告。

## ✨ V5.7.1 更新亮点

- **🐛 数据字段修复**：修复 `active_energy` 字段名（原 `active_energy_burned`），确保活动能量数据正确提取
- **🐛 睡眠数据兼容**：增强睡眠数据解析，同时兼容 `data.sleep_analysis` 和 `data.metrics[].sleep_analysis` 两种数据结构
